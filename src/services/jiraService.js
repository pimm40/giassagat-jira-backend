const { getAllProjects, getRDAByPeriod, getProjectKeyByPeriod, getProjects, getProjectsByPeriod, getIssues, getIssueByID, getUsers } = require('../../config/api');
...
const getRDAByPeriodFunc = async (RDA, startDate, endDate, pageSize, pageNumber) => {
  try {
    const data = await getRDAByPeriod(RDA, startDate, endDate, pageSize, pageNumber);
    return data;
  } catch (error) {
    throw error; // Lança o erro para ser tratado no controlador
  }
